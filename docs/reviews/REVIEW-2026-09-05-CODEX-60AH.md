...
- **Unify argument preparation around one conversion decision.** ✅ Primitive addresses, arrays, record arrays, and buffers now share conversion initializers across direct and shared allocation. `VariableGenerator.initializer(storageExpression)` varies the storage source while reusing conversion logic, including canonical scalar writes. CFString initialization also uses the same path. Strings retain `allocateFrom` for direct allocation and explicit UTF-8 copying into shared slices; records retain allocating and existing-storage converter overloads. These storage-specific differences do not require a new conversion framework.
...
- ✅ **Normalize scalar versus indexed field generation.** `FieldConversions` now shares nested-value reads and writes, pointer conversions, and nullable address expressions between scalar and indexed accessors. Both pointer getter paths delegate native-null handling to `reinterpret$F`. Field layout, indexing, and declaration validation remain in their respective generators.
- **Avoid repeated analysis inside generation.** ✅ Each method now computes one immutable allocation plan after validation, including the arena requirement, allocation list, and direct/shared choice. Source emission and invocation reuse that plan, and parameter slices use its recorded byte sizes. Reusing record-graph analysis across different methods remains a possible further optimization; no processor performance improvement has been measured.
